...
const props = defineProps<{
    thermalpintus: {
        data: Array<{ id: number; thermal_pintu: string; created_at: string }>;
        links: any[];
        from: number;
        to: number;
        total: number;
        ...
        <Card class="border-none shadow-sm">
            ...
            >
                <CardTitle class="text-xl font-bold flex items-center gap-2">
                    <IconDoor class="size-6 text-primary" />
                    Daftar Thermal Pintu
                </CardTitle>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    <div class="relative w-full md:w-72">
                        ...
                        <Input
                            v-model="search"
                            placeholder="Cari thermal pintu..."
                            class="pl-10 pr-10"
                        />
                        <button
                            v-if="search"
                    ...
                    >
                        <Link :href="route('thermalpintu.create')">
            ...
            <CardContent>
                <div class="rounded-lg border overflow-hidden">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-if="thermalpintus.data.length === 0">
                                <TableCell
                                    colspan="3"
                            ...
                            <TableRow
                                v-for="item in thermalpintus.data"
                                :key="item.id"
                            ...
                            >
                                ...
                                <TableCell class="text-right">
                                    ...
                                    >
                                        <Link
                                            :href="
                                                route('thermalpintu.edit', item.id)
                                            "
                                        >
                ...
                >
                    <p class="text-xs text-muted-foreground italic font-medium">
                        Menampilkan {{ thermalpintus.from ?? 0 }} -
                        {{ thermalpintus.to ?? 0 }} dari {{ thermalpintus.total }} data thermal pintu
                    </p>

                    <nav class="flex items-center gap-1">
                        <template v-for="(link, k) in thermalpintus.links" :key="k">
                            <Button
                                v-if="link.url === null"
                                variant="outline"
                                size="sm"
