...
const props = defineProps<{ 
    thermalpintu: { id: number; thermal_pintu: string };
}>();

const form = useForm({ 
    thermal_pintu: props.thermalpintu.thermal_pintu 
});
</script>

<template>
    <Head title="Edit Thermal Pintu" />
    <div class="flex flex-col gap-6 p-4 md:p-8 pt-1">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                ></Button>
                <h2 class="text-3xl font-bold tracking-tight">Edit Thermal Pintu</h2>
            </div>
        </div>
        <div class="max-w-2xl">
            <Card class="border-none shadow-lg">
                    ...
                    <AlertDialog>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader
                                ...
                                ><AlertDialogDescription
                                    >Hapus permanen thermal pintu
                                    <strong>{{ props.thermalpintu.thermal_pintu }}</strong>?</AlertDialogDescription
                                ></AlertDialogHeader
                            >
                            <AlertDialogFooter
                                ...
                                ><AlertDialogAction
                                    @click="
                                        router.delete(
                                            route(
                                                'thermalpintu.destroy',
                                                props.thermalpintu.id,
                                            ),
                                        )
                                    "
                ...
                <CardContent class="pt-6">
                    <form
                        @submit.prevent="
                            form.put(route('thermalpintu.update', props.thermalpintu.id))
                        "
                        class="space-y-6"
                    >
