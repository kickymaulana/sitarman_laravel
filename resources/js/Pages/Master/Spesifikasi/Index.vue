...
const props = defineProps<{
    spesifikasis: {
        data: Array<{ id: number; spesifikasi: string; created_at: string }>;
        links: any[];
        from: number;
        to: number;
        total: number;
        ...
        <Card class="border-none shadow-sm">
            ...
            >
                <CardTitle class="text-xl font-bold flex items-center gap-2">
                    <IconListDetails class="size-6 text-primary" />
                    Daftar Spesifikasi
                </CardTitle>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    >
                        <Link :href="route('spesifikasi.create')">
            ...
            <CardContent>
                <div class="rounded-lg border overflow-hidden">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-if="spesifikasis.data.length === 0">
                                <TableCell
                                    colspan="3"
                            ...
                            <TableRow
                                v-for="item in spesifikasis.data"
                                :key="item.id"
                            ...
                            >
                                ...
                                <TableCell class="text-right">
                                    ...
                                    >
                                        <Link
                                            :href="
                                                route('spesifikasi.edit', item.id)
                                            "
                                        >
                ...
                >
                    <p class="text-xs text-muted-foreground italic font-medium">
                        Menampilkan {{ spesifikasis.from ?? 0 }} -
                        {{ spesifikasis.to ?? 0 }} dari {{ spesifikasis.total }} data spesifikasi
                    </p>

                    <nav class="flex items-center gap-1">
                        <template v-for="(link, k) in spesifikasis.links" :key="k">
                            <Button
                                v-if="link.url === null"
                                variant="outline"
                                size="sm"
