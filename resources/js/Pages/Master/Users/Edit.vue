...
const props = defineProps<{
    ...
    roles: Array<{ id: number; name: string }>; // Daftar role dari Spatie
}>();
            ...
            <Card class="border-none shadow-xl ring-1 ring-emerald-100 bg-white/90 backdrop-blur-sm overflow-hidden">
                ...
                <CardHeader class="pb-8">
                    <CardTitle class="text-2xl font-black text-slate-800 flex items-center gap-2">
                        <IconUser class="size-6 text-emerald-600" />
                        Edit Profil Pengguna
                    </CardTitle>
                    <CardDescription class="text-sm font-medium">
                        Perbarui informasi dasar dan hak akses sistem untuk
                ...
                <form @submit.prevent="submit">
                    <CardContent class="grid gap-8">
                        ...
                        <div class="grid gap-2.5">
                            ...
                            <p v-if="form.errors.name" class="text-[10px] font-bold text-destructive ml-1 lowercase">
                                * {{ form.errors.name }}
                            </p>
                        </div>
                        ...
                        <div class="pt-6 mt-2 border-t border-dashed border-emerald-100">
                            <div class="flex items-center gap-2 mb-6">
                                <div class="h-4 w-1 bg-amber-400 rounded-full"></div>
                                <h3 class="text-[11px] font-black text-emerald-800 tracking-widest uppercase">
                                    Keamanan (Biarkan kosong jika tidak diganti)
                                </h3>
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="grid gap-2.5">
                                    <Label for="password" class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                                        Password Baru
                                    </Label>
                                    <div class="relative group">
                                        <IconLock class="absolute left-3 top-3 size-4 text-slate-300 group-focus-within:text-emerald-600 transition-colors" />
                                        <Input
                                            id="password"
                                ...
                                <div class="grid gap-2.5">
                                    <Label for="password_confirmation" class="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
                                        Konfirmasi Password
                                    </Label>
                                    <div class="relative group">
                                        <IconLock class="absolute left-3 top-3 size-4 text-slate-300 group-focus-within:text-emerald-600 transition-colors" />
