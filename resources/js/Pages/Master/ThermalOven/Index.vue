...
const props = defineProps<{
    thermalovens: {
        data: Array<{ id: number; thermal_oven: string; created_at: string }>;
        links: any[];
        from: number;
        to: number;
        total: number;
        ...
        <Card class="border-none shadow-sm">
            ...
            >
                <CardTitle class="text-xl font-bold flex items-center gap-2">
                    <IconTemperature class="size-6 text-primary" />
                    Daftar Thermal Oven
                </CardTitle>
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    ...
                    >
                        <Link :href="route('thermaloven.create')">
            ...
            <CardContent>
                <div class="rounded-lg border overflow-hidden">
                    <Table>
                        ...
                        <TableBody>
                            <TableRow v-if="thermalovens.data.length === 0">
                                <TableCell
                                    colspan="3"
                            ...
                            <TableRow
                                v-for="item in thermalovens.data"
                                :key="item.id"
                            ...
                            >
                                ...
                                <TableCell class="text-right">
                                    ...
                                    >
                                        <Link
                                            :href="
                                                route('thermaloven.edit', item.id)
                                            "
                                        >
                ...
                >
                    <p class="text-xs text-muted-foreground italic font-medium">
                        Menampilkan {{ thermalovens.from ?? 0 }} -
                        {{ thermalovens.to ?? 0 }} dari {{ thermalovens.total }} data thermal oven
                    </p>

                    <nav class="flex items-center gap-1">
                        <template v-for="(link, k) in thermalovens.links" :key="k">
                            <Button
                                v-if="link.url === null"
                                variant="outline"
                                size="sm"
