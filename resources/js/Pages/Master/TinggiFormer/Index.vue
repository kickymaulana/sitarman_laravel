...
const props = defineProps<{
    tinggiformers: {
        data: Array<{ id: number; tinggi_former: number; created_at: string }>;
        links: any[];
        from: number;
        to: number;
        total: number;
            ...
            <CardHeader class="flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0 pb-6">
                ...
                <div class="flex items-center gap-2 w-full md:w-auto">
                    <div class="relative w-full md:w-72">
                        ...
                        <Input
                            v-model="search"
                            placeholder="Cari tinggi former..."
                            class="pl-10 pr-10"
                        />
                        <button
                            v-if="search"
                    ...
                    <Button as-child class="bg-primary hover:bg-primary/90 shadow-md transition-all active:scale-95">
                        <Link :href="route('tinggiformer.create')">
            ...
            <CardContent>
                            ...
                            >
                                ...
                                <TableCell class="text-right">
                                    <Button variant="ghost" size="icon" class="size-8 hover:text-primary transition-colors" as-child>
                                        <Link :href="route('tinggiformer.edit', item.id)">
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-4 mt-6">
                    <p class="text-xs text-muted-foreground italic font-medium">
                        Menampilkan {{ tinggiformers.from ?? 0 }} - {{ tinggiformers.to ?? 0 }} dari {{ tinggiformers.total }} data
                    </p>

                    <nav class="flex items-center gap-1">
                        <template v-for="(link, k) in tinggiformers.links" :key="k">
                            <Button
                                v-if="link.url === null"
                                variant="outline"
                                size="sm"
