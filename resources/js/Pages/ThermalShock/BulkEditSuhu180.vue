...
const submit = () => {
    form.put(route('thermalshock.bulkUpdate180'), {
        preserveScroll: true,
    });
};
...
<template>
    <Head title="Bulk Edit Parameter Suhu 180°C" />
    ...
    <div class="max-w-3xl mx-auto p-4 md:p-8">
        ...
        <Card class="shadow-md border border-blue-100 dark:border-blue-950">
            <CardHeader class="space-y-1 bg-blue-50/50 dark:bg-blue-950/10 pb-6 rounded-t-lg">
                <CardTitle class="text-lg font-bold flex items-center gap-2 text-blue-700 dark:text-blue-400">
                    <IconFlame class="size-5" />
                    Isi Parameter & Waktu Massal Suhu 180°C
                </CardTitle>
                <CardDescription>
            ...
            <CardContent class="pt-6">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="space-y-3">
                        <h4 class="text-xs font-bold uppercase tracking-wider text-blue-600 dark:text-blue-400">2. Parameter Waktu Proses (HH:mm)</h4>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 bg-blue-50/10 dark:bg-blue-950/5 p-4 border rounded-lg">
                            <div class="space-y-1.5">
                                ...
                                <Input type="text" v-model="form.jam_awal_proses_180" @input="formatTimeInput('jam_awal_proses_180', $event)" placeholder="00:00" class="h-9 text-center font-mono" />
                                <span v-if="form.errors.jam_awal_proses_180" class="text-xs text-destructive">{{ form.errors.jam_awal_proses_180 }}</span>
                            </div>

                            <div class="space-y-1.5">
                                <Label class="text-xs">Jam Capai Suhu</Label>
                                <Input type="text" v-model="form.jam_capai_suhu_180" @input="formatTimeInput('jam_capai_suhu_180', $event)" placeholder="00:00" class="h-9 text-center font-mono" />
                                <span v-if="form.errors.jam_capai_suhu_180" class="text-xs text-destructive">{{ form.errors.jam_capai_suhu_180 }}</span>
                            </div>

                            <div class="space-y-1.5">
                                <Label class="text-xs">Jam Mulai Tembak</Label>
                                <Input type="text" v-model="form.jam_mulai_tembak_180" @input="formatTimeInput('jam_mulai_tembak_180', $event)" placeholder="00:00" class="h-9 text-center font-mono" />
                                <span v-if="form.errors.jam_mulai_tembak_180" class="text-xs text-destructive">{{ form.errors.jam_mulai_tembak_180 }}</span>
                            </div>

                            <div class="space-y-1.5">
                                <Label class="text-xs">Jam Selesai Tembak</Label>
                                <Input type="text" v-model="form.jam_selesai_tembak_180" @input="formatTimeInput('jam_selesai_tembak_180', $event)" placeholder="00:00" class="h-9 text-center font-mono" />
                                <span v-if="form.errors.jam_selesai_tembak_180" class="text-xs text-destructive">{{ form.errors.jam_selesai_tembak_180 }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-2 pt-2 border-t">
                        ...
                        <Button type="submit" :disabled="form.processing" class="bg-blue-600 hover:bg-blue-700 text-white text-xs">
